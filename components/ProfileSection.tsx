...
interface ProfileProps {
  ...
  label: string;
  defaultColor: string;
  driveImageUrl: string; // Add this new prop
}

const ProfileCard: React.FC<ProfileProps> = ({ id, label, defaultColor, driveImageUrl }) => {
  // We initialize the state with the driveImageUrl as the fallback
  const [image, setImage] = useState<string>(driveImageUrl);
  const fileInputRef = useRef<HTMLInputElement>(null);

  useEffect(() => {
    const savedImage = localStorage.getItem(`profile_photo_${id}`);
    // Only override the drive image if the user has uploaded something locally
  ...
  return (
    <div className="flex flex-col items-center group">
      ...
      >
        {/* The image will now always have a source (either Drive or LocalStorage) */}
